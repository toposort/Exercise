/************************************
有n种插座,每种插座一个,插座只能插一个电器(或者适配器)
有m种电器，每种电器有一个插头需要插在相应一种插座上
有k种适配器，每种适配器可以有无限多数量
每种适配器(a, b)可以把b类插座变为a类插座
问最后有多少个电器无法使用
各个电器各自为一个节点,和源点相连,容量为1
将室内已有的插座各自为一个节点,和汇点相连,容量为1
将电器到其应该插的插座类型的点做边,容量为1
将适配器(a, b)从a点到b点连边,容量为oo
*************************************/
